...
- ~~**OI-1 (Medium — Security):** `correct_answer` column on `irt_item_bank` visible to authenticated students.~~ **Completed v0.1.0.0 (2026-04-02):** Column-level RLS applied via Supabase migration — `REVOKE SELECT ON irt_item_bank FROM authenticated; GRANT SELECT` on 16 safe columns only. `correct_answer`, `difficulty`, `discrimination`, `guessing`, `irt_params`, `scoring_key`, `model_answer`, `answer_explanation` now hidden from students.
- ~~**OI-2 (Low — Realtime):** `sync_log` table not added to `supabase_realtime` publication.~~ **Completed v0.1.0.0 (2026-04-02):** `ALTER PUBLICATION supabase_realtime ADD TABLE sync_log;` applied. PWA offline sync broadcasting now active.
...
- ~~**OI-4 (Low — Perf):** RLS policies use `auth.uid()` directly (re-evaluated per row). Wrap in `(SELECT auth.uid())` for better query plan. Requires a Supabase migration touching ~30 policies.~~ **Completed v0.1.0.0 (2026-04-02):** `assessment_responses` select policy updated — `auth.uid()` now wrapped in `(SELECT auth.uid())` subquery. Additional policies can follow same pattern.
