...
export async function POST(request: Request) {
  try {
    ...
    if (!isAllowed) {
      return new Response(
        "Rate limit exceeded. Please wait before making another TTS request.",
        {
          status: 429,
        },
      );
    ...
    if (text.length > AI_CONTENT_LIMITS.ttsMaxLength) {
      return new Response(
        `Text is too long (max ${AI_CONTENT_LIMITS.ttsMaxLength} characters)`,
        { status: 400 },
      );
    }
    ...
    if (error instanceof Error) {
      if (error.message.includes("model is loading")) {
        return new Response(
          "TTS model is loading. Please try again in a few seconds.",
          {
            status: 503,
          },
        );
