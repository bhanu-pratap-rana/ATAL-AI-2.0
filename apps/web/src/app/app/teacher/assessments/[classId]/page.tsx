...
export default async function ClassAssessmentResultsPage({ params }: PageProps) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-cream to-surface page-layout">
      <div className="container-responsive max-w-6xl">
        ...
        <div className="mb-responsive">
          <Link href="/app/teacher/assessments" className="text-primary hover:text-primary-dark mb-4 inline-flex items-center gap-1 text-sm md:text-base touch-target">
            ← Back to Assessments
          </Link>
          <h1 className="heading-1 text-primary mb-2">{results.className}</h1>
          <p className="text-text-secondary text-sm md:text-base">Student Assessment Results</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-responsive">
          <Card className="card-responsive bg-primary/10 border-primary/20">
            <CardContent className="py-4 text-center">
              <p className="text-2xl font-bold text-primary">{results.totalStudents}</p>
              <p className="text-xs text-text-secondary">Total Students</p>
            </CardContent>
          </Card>
          <Card className="card-responsive bg-info/10 border-info/20">
            <CardContent className="py-4 text-center">
              <p className="text-2xl font-bold text-info-dark">{results.studentsWithAssessments}</p>
          ...
          <Card className="card-responsive bg-success/10 border-success/20">
            <CardContent className="py-4 text-center">
              <p className="text-2xl font-bold text-success-dark">
                {results.classAverageScore !== null ? `${results.classAverageScore}%` : '-'}
              </p>
          ...
          <CardContent>
            {hasStudents ? (
                      ...
                      >
                        <div className="flex items-center justify-between mb-3">
                          <div>
                            ...
                            {student.rollNumber && (
                              <p className="text-xs text-text-secondary">Roll: {student.rollNumber}</p>
                            )}
                          </div>
                          <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold ${getScoreColor(student.averageScore)}`}>
                            {student.averageScore !== null ? `${student.averageScore}%` : '-'}
                          </div>
                        </div>
                        <div className="flex flex-wrap items-center gap-2 text-xs">
                          <span className={`px-2 py-0.5 rounded-full ${skillLevel.color}`}>
                            {skillLevel.label}
                          </span>
                        ...
                        {student.lastAssessmentDate && (
                          <p className="text-xs text-text-tertiary mt-2">
                            Last: {formatRelativeTime(student.lastAssessmentDate)}
                          </p>
                        )}
                      </div>
                    )
                ...
                <table className="hidden md:table w-full">
                  <thead>
                    <tr className="border-b border-border">
                      ...
                      <th className="text-left py-3 px-4 text-sm font-medium text-text-secondary">Roll #</th>
                      <th className="text-center py-3 px-4 text-sm font-medium text-text-secondary">Score</th>
                      <th className="text-center py-3 px-4 text-sm font-medium text-text-secondary">Level</th>
                      <th className="text-center py-3 px-4 text-sm font-medium text-text-secondary">Sessions</th>
                      <th className="text-center py-3 px-4 text-sm font-medium text-text-secondary">Correct</th>
                      <th className="text-right py-3 px-4 text-sm font-medium text-text-secondary">Last Activity</th>
                    </tr>
                  </thead>
                  <tbody>
                    {results.results.map((student) => {
                      const skillLevel = getSkillLevel(student.averageScore)
                      return (
                        <tr key={student.studentId} className="border-b border-border-light hover:bg-surface/50">
                          ...
                          <td className="py-3 px-4 text-center">
                            <span className={`inline-flex items-center justify-center w-12 h-8 rounded font-bold text-sm ${getScoreColor(student.averageScore)}`}>
                              {student.averageScore !== null ? `${student.averageScore}%` : '-'}
                            </span>
                          </td>
                          <td className="py-3 px-4 text-center">
                            <span className={`inline-block px-2 py-0.5 rounded-full text-xs ${skillLevel.color}`}>
                              {skillLevel.label}
                            </span>
                          </td>
                          ...
                          <td className="py-3 px-4 text-center text-text-secondary">
                            {student.totalQuestions > 0 ? `${student.correctAnswers}/${student.totalQuestions}` : '-'}
                          </td>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-text-tertiary text-base mb-2">No students enrolled yet</p>
        ...
        <Card className="card-responsive mt-responsive">
          <CardContent className="py-4">
            <p className="text-sm font-medium text-text-secondary mb-3">Score Levels</p>
            <div className="flex flex-wrap gap-4">
              <div className="flex items-center gap-2">
                <span className="w-4 h-4 rounded bg-success"></span>
                <span className="text-sm text-text-secondary">Advanced (80%+)</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-4 h-4 rounded bg-warning"></span>
                <span className="text-sm text-text-secondary">Intermediate (60-79%)</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-4 h-4 rounded bg-error"></span>
                <span className="text-sm text-text-secondary">Beginner (&lt;60%)</span>
              </div>
            </div>
          </CardContent>
        </Card>
