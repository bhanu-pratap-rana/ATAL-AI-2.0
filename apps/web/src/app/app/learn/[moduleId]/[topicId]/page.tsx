...
interface LessonContent {
  ...
  title_as: string;
  sections: Array<{
    type: 'text' | 'image' | 'video' | 'activity' | 'curriculum' | 'definition' | 'example' | 'exercise' | 'cultural_context';
    content: string;
    image_url?: string;
  }>;
...
const DEFAULT_LESSON: LessonContent = {
  ...
  sections: [
    {
      type: 'text',
      content: 'This lesson content is being prepared. In the meantime, you can ask the AI Tutor any questions about this topic!',
    },
  ],
  practice_questions: [],
};
...
export default function LessonPage() {
  ...
  useEffect(() => {
    const fetchLessonContent = async () => {
      try {
        ...
        if (contentError) {
          clientLogger.error('[LessonPage] Error fetching lesson content', contentError instanceof Error ? contentError : { error: String(contentError) });
        ...
        clientLogger.error('[LessonPage] Error fetching lesson', error instanceof Error ? error : { error: String(error) });
        setLesson(DEFAULT_LESSON);
        setLoading(false);
      }
    };
  ...
  const handlePracticeSubmit = async () => {
    ...
    try {
      ...
      clientLogger.error('[LessonPage] Error saving practice responses', error instanceof Error ? error : { error: String(error) });
    }
  };
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      clientLogger.error('[LessonPage] Error updating knowledge state', error instanceof Error ? error : { error: String(error) });
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/30">
      <div className="flex">
        {/* Main Content Area */}
        <main className={`flex-1 p-4 md:p-6 transition-all ${showAITutor ? 'mr-96' : ''}`}>
          <div className="max-w-3xl mx-auto space-y-6">
            {/* Back Link */}
            <Link
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-2xl">{lesson.title_en}</CardTitle>
                            ...
                            <button
                              ...
                              onClick={() => {
                                if (!practiceSubmitted) {
                                  setPracticeAnswers({ ...practiceAnswers, [q.id]: oIdx });
                                }
                              }}
