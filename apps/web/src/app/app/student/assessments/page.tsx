...
function getSkillLevel(score: number): { label: string; color: string } {
  ...
  if (score >= 60) return { label: 'Intermediate', color: 'bg-warning text-white' }
  ...
  if (sessions && sessions.length > 0) {
    for (const session of sessions) {
      ...
      if (responses) {
        ...
        const timeSpent = Math.round(responses.reduce((sum, r) => sum + (r.rt_ms || 0), 0) / 1000)
    ...
    <div className="min-h-screen bg-gradient-to-br from-cream to-surface page-layout">
      <div className="container-responsive max-w-6xl">
        ...
        <div className="mb-responsive">
          ...
          <p className="text-text-secondary text-sm md:text-base">View and complete your assessments</p>
        </div>
        ...
        <Card className="card-responsive mb-responsive bg-gradient-to-br from-primary/10 to-primary/5 border-primary/20">
          <CardContent className="py-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                <h2 className="text-lg font-bold text-primary mb-1">Start a New Assessment</h2>
                <p className="text-text-secondary text-sm">
                  Test your digital literacy skills with our comprehensive assessment
                </p>
              </div>
          ...
          <CardContent>
            {hasHistory ? (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <span className="font-medium text-text-primary">Digital Literacy Assessment</span>
                            <span className={`text-xs px-2 py-0.5 rounded-full ${skillLevel.color}`}>
                              {skillLevel.label}
                            </span>
                          </div>
                          <p className="text-sm text-text-secondary">
                            {assessment.correctAnswers}/{assessment.totalQuestions} correct • {formatTime(assessment.timeSpent)}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 sm:flex-col sm:items-end">
                        ...
                        <Link href={`/app/assessment/summary?session=${assessment.id}`}>
              ...
              <div className="text-center py-8 md:py-12">
                ...
                <p className="text-text-tertiary text-base md:text-lg font-medium">No assessments completed yet</p>
                <p className="text-text-secondary text-sm mt-2 px-4 max-w-md mx-auto">
                  Take your first assessment to test your digital literacy skills and track your progress over time.
                </p>
                <Link href="/app/assessment/start">
            ...
            <Card className="card-responsive bg-success/10 border-success/20">
              <CardContent className="py-4 text-center">
                <p className="text-2xl font-bold text-success-dark">
                  {Math.round(assessmentHistory.reduce((sum, a) => sum + a.score, 0) / assessmentHistory.length)}%
                </p>
