...
export default async function ProgressPage() {
  ...
  const hasData = stats && (stats.assessmentsTaken > 0 || stats.moduleBreakdown.length > 0)
    ...
    <div className="min-h-screen bg-gradient-to-br from-cream to-surface page-layout">
      <div className="container-responsive max-w-6xl">
        ...
        <div className="mb-responsive text-center sm:text-left">
          ...
          <p className="text-text-secondary text-sm md:text-base">Track your learning journey and performance</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-responsive mb-responsive">
          <Card className="bg-gradient-to-br from-primary/20 to-primary/10 border-primary/30 card-responsive">
            <CardHeader className="pb-2">
              <CardTitle className="text-primary-dark text-sm md:text-base">Courses Completed</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-3xl md:text-4xl font-bold text-primary">
                {stats?.coursesCompleted ?? 0}
              </p>
              <p className="text-xs text-text-secondary mt-1">Score ≥60% to complete</p>
            </CardContent>
          </Card>

          <Card className="bg-gradient-to-br from-accent-light to-accent-light/50 border-accent/30 card-responsive">
            <CardHeader className="pb-2">
              <CardTitle className="text-accent-dark text-sm md:text-base">Assessments Taken</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-3xl md:text-4xl font-bold text-accent-dark">
                {stats?.assessmentsTaken ?? 0}
              </p>
              <p className="text-xs text-text-secondary mt-1">Total completed</p>
            </CardContent>
          </Card>

          <Card className="bg-gradient-to-br from-success-light to-success-light/50 border-success/30 card-responsive">
            <CardHeader className="pb-2">
              <CardTitle className="text-success-dark text-sm md:text-base">Average Score</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-3xl md:text-4xl font-bold text-success-dark">
                {stats?.averageScore != null ? `${stats.averageScore}%` : '--'}
              </p>
              <p className="text-xs text-text-secondary mt-1">Across all attempts</p>
            </CardContent>
          </Card>

          <Card className="bg-gradient-to-br from-info-light to-info-light/50 border-info/30 card-responsive">
            <CardHeader className="pb-2">
              <CardTitle className="text-info-dark text-sm md:text-base">Time Spent</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-3xl md:text-4xl font-bold text-info-dark">
                {stats?.totalTimeSpent ? formatTime(stats.totalTimeSpent) : '--'}
              </p>
              <p className="text-xs text-text-secondary mt-1">Total learning time</p>
            </CardContent>
          </Card>
        </div>
        ...
        {hasData && stats.moduleBreakdown.length > 0 && (
          <Card className="card-responsive mb-responsive">
            <CardHeader>
              <CardTitle className="text-lg md:text-xl">Module Performance</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                {stats.moduleBreakdown.map((module) => (
                  <div key={module.module}>
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-sm font-medium text-text-primary">{module.module}</span>
                    ...
                    <div className="w-full bg-border rounded-full h-2.5">
                      <div
                        className={`h-2.5 rounded-full transition-all duration-500 ${
                          module.averageScore >= 80 ? 'bg-success' :
                          module.averageScore >= 60 ? 'bg-warning' : 'bg-error'
                        }`}
          ...
          <CardContent>
            {hasData && stats.recentAssessments.length > 0 ? (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold ${
                        assessment.score >= 80 ? 'bg-success' :
                        assessment.score >= 60 ? 'bg-warning' : 'bg-error'
                      }`}>
                        {assessment.score}%
                      </div>
                      <div>
                        <p className="font-medium text-text-primary">Assessment Completed</p>
              ...
              <div className="text-center py-8 md:py-12">
                ...
                <p className="text-text-tertiary text-base md:text-lg font-medium">No activity yet</p>
                <p className="text-text-secondary text-sm mt-2 px-4 max-w-md mx-auto">
                  Take your first assessment to start tracking your learning progress and see detailed performance metrics here.
                </p>
                <Link
