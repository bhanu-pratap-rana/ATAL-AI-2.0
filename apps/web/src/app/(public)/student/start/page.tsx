...
export default function StudentStartPage() {
  ...
  async function handleSaveProfile(e: React.FormEvent) {
    ...
    try {
      ...
      try {
        ...
        if (refreshError) {
          authLogger.warn(
            "[SaveProfile] Session refresh failed, continuing anyway",
            refreshError,
          );
        }
      } catch (refreshErr) {
        authLogger.warn(
          "[SaveProfile] Session refresh exception",
  ...
  async function handleUsernameSignup(e: React.FormEvent) {
    ...
    try {
      ...
      const availabilityCheck = await checkUsernameAvailable(
        state.signupUsername,
      );
      if (!availabilityCheck.available) {
        actions.setSignupUsernameError(
          availabilityCheck.error || "Username is not available",
        );
        actions.setIsLoading(false);
        return;
      }
          ...
          {state.signinTab === "email" && (
            <form onSubmit={handleSignInEmail} className="space-y-4">
              ...
              <Button
                ...
                disabled={
                  state.isLoading ||
                  !state.signinEmailAddress ||
                  !state.signinEmailPassword
                }
          ...
          {state.signinTab === "phone" && (
            <form onSubmit={handleSignInPhone} className="space-y-4">
              ...
              <Button
                ...
                disabled={
                  state.isLoading ||
                  signinPhoneInput.displayValue.length < PHONE_DIGIT_LENGTH ||
        ...
        <div className="space-y-4">
            ...
            <>
              {state.signupEmailOtpSent ? (
                ...
                >
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="signup-email-password-confirm">
                      Confirm Password
                    </Label>
                    <Input
                ...
                <form onSubmit={handleSignUpPhoneSendOtp} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center border border-input rounded-md">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          signupPhoneInput.onChange(e.target.value)
                        }
                        required
                        disabled={state.isLoading}
                        className="border-0 flex-1"
                  ...
                  <div className="bg-cyan-lightest border-l-4 border-cyan p-3 rounded-xl">
                    <p className="text-xs text-cyan-darkest">
                      <strong>📱 SMS Verification:</strong> You&apos;ll receive
                      a 6-digit code via SMS. Standard rates may apply.
                    </p>
                  </div>
                  ...
                  <Button
                    ...
                    disabled={
                      state.isLoading ||
                      signupPhoneInput.displayValue.length < PHONE_DIGIT_LENGTH
                    }
                    loading={state.isLoading}
                  >
                    Send OTP
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        actions.setSignupPhonePassword(e.target.value)
                      }
                      required
                      disabled={state.isLoading}
                    />
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="signup-phone-password-confirm">
                      Confirm Password
                    </Label>
                    <Input
          ...
          {state.signupTab === "guest" && (
            <form onSubmit={handleUsernameSignup} className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="signup-username-password-confirm">
                  Confirm Password
                </Label>
                <Input
              ...
              <div className="bg-cyan-lightest border-l-4 border-cyan p-3 rounded-xl">
                <p className="text-xs text-cyan-darkest">
                  <strong>💡 No email needed!</strong> Create an account with
                  just a username and password. Remember your username for
                  future logins.
                </p>
              </div>
            ...
            <form onSubmit={handleResetPassword} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    forgotPasswordOtpInput.onChange(e.target.value)
                  }
                  required
                  disabled={state.isLoading}
                  maxLength={OTP_LENGTH}
        ...
        <form onSubmit={handleSaveProfile} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                actions.setProfilePhone(sanitizeProfilePhone(e.target.value))
              }
              disabled={state.isLoading}
              maxLength={10}
            />
            ...
            {state.profilePhone &&
              state.profilePhone.length > 0 &&
              state.profilePhone.length < 10 && (
                <p className="text-xs text-warning">
                  {10 - state.profilePhone.length} more digits needed
                </p>
              )}
          </div>
          ...
          <Button
            ...
            className="w-full text-[17px] shadow-[var(--shadow-primary)] hover:shadow-[var(--shadow-primary-hover)] hover:-translate-y-0.5"
            disabled={
              state.isLoading || !state.profileName || !state.profileGender
            }
            loading={state.isLoading}
          >
