...
export async function getAdaptiveQuestions(language: 'en' | 'hi' | 'as' = 'en') {
  try {
    ...
    // Convert to typed IRTItem array
    const itemPool: IRTItem[] = allItems.map(item => ({
      id: item.id,
    ...
    // Adaptive question selection using a-Stratified MFI
    ...
    // Select 30 questions adaptively
    for (let i = 0; i < CAT_CONFIG.TOTAL_QUESTIONS; i++) {
      ...
      if (i % 2 === 0) {
        ...
        // Simulate incorrect answer -> decrease theta slightly
        currentTheta = Math.max(currentTheta - 0.1, CAT_CONFIG.THETA_BOUNDS.min)
      }
    }

    // Verify content coverage - ensure minimum questions per category
    ...
    // Fill gaps if needed (fallback to ensure content validity)
    ...
    // Shuffle questions to interleave categories for better test experience
    const shuffledQuestions = shuffleWithinCategories(selectedQuestions)

    // Format for frontend (protect correct answers)
    const formattedQuestions = shuffledQuestions.map((q, index) => ({
      ...
      options: q.options,
      // Store correct answer index for validation (1-indexed in DB)
      _correctIndex: q.correct_answer,
      // IRT parameters for real-time adaptive scoring
    ...
    return {
      ...
      // Return metadata for frontend adaptive logic
      catConfig: {
...
export async function calculateIRTScore(
  ...
  // Calculate category-level proficiency
  ...
  const getProficiencyLevel = (t: number): string => {
  ...
  // Convert theta to percentage score (normalized 0-100)
  const thetaToPercent = (t: number): number => {
