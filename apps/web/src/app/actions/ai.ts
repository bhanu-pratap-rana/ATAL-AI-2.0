...
export async function askAITutor(
  ...
  const isAllowed = await checkRateLimit(`ai:tutor:${auth.userId}`, AI_RATE_LIMITS.tutor)
  if (!isAllowed) {
    authLogger.warn('[askAITutor] Rate limit exceeded', { userId: auth.userId })
    return { success: false, error: 'Too many requests. Please wait before asking another question.' }
  }

  if (!question || question.trim().length < AI_CONTENT_LIMITS.questionMinLength) {
    return { success: false, error: 'Please provide a valid question' }
  }

  if (question.length > AI_CONTENT_LIMITS.questionMaxLength) {
    return { success: false, error: `Question is too long (max ${AI_CONTENT_LIMITS.questionMaxLength} characters)` }
  }
  ...
  try {
    ...
    if (!response.success) {
      return { success: false, error: response.error || 'Failed to get response from AI' }
    }
...
export async function getAIEssayFeedback(
  ...
  // Rate limit check - prevent abuse
  const isAllowed = await checkRateLimit(`ai:essay:${auth.userId}`, AI_RATE_LIMITS.essay)
  if (!isAllowed) {
    authLogger.warn('[getAIEssayFeedback] Rate limit exceeded', { userId: auth.userId })
    return { success: false, error: 'Too many requests. Please wait before submitting another essay.' }
  }

  if (!essay || essay.trim().length < AI_CONTENT_LIMITS.essayMinLength) {
    return { success: false, error: `Essay must be at least ${AI_CONTENT_LIMITS.essayMinLength} characters` }
  }

  if (essay.length > AI_CONTENT_LIMITS.essayMaxLength) {
    return { success: false, error: `Essay is too long (max ${AI_CONTENT_LIMITS.essayMaxLength} characters)` }
  }
  ...
  try {
    ...
    if (!response.success) {
      return { success: false, error: response.error || 'Failed to analyze essay' }
    }
...
export async function generateAIPracticeQuestions(
  ...
  // Rate limit check - prevent abuse
  const isAllowed = await checkRateLimit(`ai:practice:${auth.userId}`, AI_RATE_LIMITS.practice)
  if (!isAllowed) {
    authLogger.warn('[generateAIPracticeQuestions] Rate limit exceeded', { userId: auth.userId })
    return { success: false, error: 'Too many requests. Please wait before generating more questions.' }
  }
  ...
  if (count < AI_CONTENT_LIMITS.practiceQuestionsMin || count > AI_CONTENT_LIMITS.practiceQuestionsMax) {
    return { success: false, error: `Number of questions must be between ${AI_CONTENT_LIMITS.practiceQuestionsMin} and ${AI_CONTENT_LIMITS.practiceQuestionsMax}` }
  }

  try {
    const response = await generatePracticeQuestions(topic.trim(), count, difficulty, language)

    if (!response.success) {
      return { success: false, error: response.error || 'Failed to generate questions' }
    }
...
export async function summarizeStudyContent(
  ...
  // Rate limit check - prevent abuse
  const isAllowed = await checkRateLimit(`ai:summarize:${auth.userId}`, AI_RATE_LIMITS.summarize)
  if (!isAllowed) {
    authLogger.warn('[summarizeStudyContent] Rate limit exceeded', { userId: auth.userId })
    return { success: false, error: 'Too many requests. Please wait before summarizing more content.' }
  }

  if (!content || content.trim().length < AI_CONTENT_LIMITS.contentMinLength) {
    return { success: false, error: `Content must be at least ${AI_CONTENT_LIMITS.contentMinLength} characters` }
  }

  if (content.length > AI_CONTENT_LIMITS.contentMaxLength) {
    return { success: false, error: `Content is too long (max ${AI_CONTENT_LIMITS.contentMaxLength} characters)` }
  }
  ...
  try {
    ...
    if (!response.success) {
      return { success: false, error: response.error || 'Failed to summarize content' }
    }
