...
import { normalizeSchoolCode, handleZodValidationError, type VerifyTeacherParams, type VerifyTeacherResult, type SchoolData } from './school-utils'
...
async function lookupSchoolByCode(
  adminClient: Awaited<ReturnType<typeof createAdminClient>>,
  schoolCode: string
): Promise<{ success: true; school: SchoolData } | { success: false; error: string }> {
...
export async function verifyTeacher({
  ...
  try {
    ...
    const isAllowed = await checkRateLimit(`verify-teacher:${user.id}`, RATE_LIMITS.teacherVerification)
    if (!isAllowed) {
