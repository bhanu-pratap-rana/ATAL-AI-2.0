...
export class CurriculumRAGService {
  ...
  async getMultilingualContext(
    ...
    const { matchThreshold = 0.6, matchCount = 5, filterTopic = null } = options;
    ...
    try {
      ...
      const { data: primaryDocs, error: primaryError } = await supabase.rpc('match_curriculum', {
        query_embedding: embedding,
        match_threshold: matchThreshold,
        match_count: matchCount,
      ...
      const { data: allDocs, error: allError } = await supabase.rpc('match_curriculum', {
        query_embedding: embedding,
  ...
  async hybridSearch(
    ...
    try {
      ...
      const { data: docs, error } = await supabase.rpc('match_curriculum_hybrid', {
        query_embedding: embedding,
  ...
  async getTopicContext(
    ...
    try {
      ...
      if (!error && docs && docs.length > 0) {
        return docs.map((d: { title: string; content: string }) =>
          `### ${d.title}\n${d.content}`
        ).join('\n\n');
      }
  ...
  private async getDirectTopicContent(
    ...
    try {
      ...
      if (language !== 'en') {
        authLogger.warn(`[RAG] No content found for topic ${topicId} in ${language}, trying English fallback`);
  ...
  private async getEmbedding(text: string): Promise<number[]> {
    ...
    if (!apiKey) {
      throw new Error('GEMINI_API_KEY or GOOGLE_GENERATIVE_AI_API_KEY is required for embeddings');
    }
  ...
  private formatContext(docs: CurriculumMatch[]): string {
    ...
    const langLabels = languages.map(l => this.getLanguageLabel(l as 'en' | 'hi' | 'as')).join(', ');

    const contextParts = docs.map((doc, index) => {
      const header = doc.title ? `### ${doc.title}` : `### Context ${index + 1}`;
      const langLabel = this.getLanguageLabel(doc.language as 'en' | 'hi' | 'as');
