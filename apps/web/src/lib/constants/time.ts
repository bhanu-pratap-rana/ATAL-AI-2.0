...
/** `n` days in milliseconds — for `Date.now() + DAYS(7)` style code. */
export const DAYS = (n: number) => n * MS_PER_DAY;
/** `n` hours in milliseconds. */
export const HOURS = (n: number) => n * MS_PER_HOUR;
/** `n` minutes in milliseconds. */
export const MINUTES = (n: number) => n * MS_PER_MINUTE;
