...
-- practice_questions: Will be handled in consolidation below
...
-- Note: irt_item_bank_admin_all already handles INSERT/UPDATE/DELETE for admins
-- We keep it as is since it's an ALL policy, not just SELECT
...
-- Note: practice_questions is a reference table (no student_id), so all authenticated users can read
...
-- Check that all policies now use TO authenticated or TO service_role
-- This query should return 0 rows if all anonymous access is removed
SELECT 
  schemaname,
  tablename,
  policyname,
...
WHERE schemaname IN ('public', 'auth')
  AND tablename IN (
    'ai_tutor_interactions', 'badges', 'classes', 'formative_responses',
    'irt_item_bank', 'learning_style_profile', 'points_history', 
    'practice_questions', 'student_badges', 'student_knowledge_state',
    'student_profiles', 'summative_results', 'users', 'teacher_profiles',
    'usernames', 'assessment_responses', 'assessment_sessions',
    'curriculum_content', 'enrollments', 'schools'
  )
  AND roles && ARRAY['public', 'anon']::text[]
