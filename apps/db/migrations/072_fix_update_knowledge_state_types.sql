-- Migration 072: Fix type mismatch in update_knowledge_state RPC function
-- Purpose: Change p_module_id and p_topic_id from UUID to TEXT to match table schema
-- Root Cause: Migration 053 used UUID parameters, but student_knowledge_state table uses TEXT columns
-- Impact: Function was never callable, resulting in 0 rows in student_knowledge_state (adaptive learning blind)

-- Drop existing function with UUID parameters
DROP FUNCTION IF EXISTS update_knowledge_state(uuid, uuid, uuid, boolean, integer, boolean);

-- Recreate with TEXT parameters (matching table schema in migration 042)
CREATE OR REPLACE FUNCTION update_knowledge_state(
  p_student_id uuid,
  p_module_id text,   -- FIXED: uuid → text
  p_topic_id text,    -- FIXED: uuid → text
...
-- Grant execute to authenticated users (with corrected signature)
GRANT EXECUTE ON FUNCTION update_knowledge_state(uuid, text, text, boolean, integer, boolean) TO authenticated;

-- Add helpful comment
COMMENT ON FUNCTION update_knowledge_state(uuid, text, text, boolean, integer, boolean) IS
'Atomic knowledge state update RPC function with FIXED parameter types. Implements BKT-inspired mastery calculation with atomic locking to prevent concurrent update race conditions. Includes forgetting curve calculation and returns updated knowledge state. FIXED: Changed p_module_id and p_topic_id from UUID to TEXT to match table schema.';
