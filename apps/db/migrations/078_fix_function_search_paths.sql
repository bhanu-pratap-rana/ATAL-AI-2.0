...
BEGIN
  ...
  -- Build module breakdown using subquery to avoid nested aggregates
...
BEGIN
  ...
  -- Calculate learning rate (higher for early attempts)
  v_learning_rate := CASE
    WHEN COALESCE(v_current_state.attempts, 0) < 3 THEN 0.3
    WHEN COALESCE(v_current_state.attempts, 0) < 10 THEN 0.2
    ELSE 0.1
  END;

  -- Adjust for AI hint usage (reduces learning gain)
  IF p_ai_hint_requested THEN
    v_learning_rate := v_learning_rate * 0.7;
  END IF;

  -- Simple IRT-inspired update
  IF p_is_correct THEN
    v_new_mastery := v_current_mastery + (1 - v_current_mastery) * v_learning_rate;
  ELSE
    v_new_mastery := v_current_mastery * (1 - v_learning_rate * 0.5);
  END IF;
  ...
  v_confidence_level := CASE
    WHEN v_new_mastery >= 0.7 THEN 'high'
    WHEN v_new_mastery >= 0.4 THEN 'medium'
    ELSE 'low'
  END;

  -- Determine status
  v_new_status := CASE
    WHEN v_new_mastery >= 0.85 THEN 'mastered'
    WHEN v_new_mastery >= 0.7 THEN 'completed'
    WHEN v_new_mastery > 0 THEN 'in_progress'
    ELSE 'not_started'
  END;
  ...
  RETURN jsonb_build_object(
    ...
    'mastery_score', v_new_mastery,
    'confidence_level', v_confidence_level,
    'attempts', COALESCE(v_current_state.attempts, 0) + 1,
    'status', v_new_status,
...
COMMENT ON FUNCTION submit_assessment IS 'Atomically submits assessment responses and updates knowledge state. Search path fixed for security.';
COMMENT ON FUNCTION update_knowledge_state IS 'Updates student knowledge state using IRT-inspired algorithm. Search path fixed for security.';
