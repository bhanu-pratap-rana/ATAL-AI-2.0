-- Migration 074: Fix item_id casting bug in submit_assessment
-- Bug: Migration 073 casts itemId to ::uuid, but assessment_responses.item_id is TEXT
-- Fix: Remove ::uuid cast since item_id column is TEXT
-- Impact: submit_assessment was failing silently on all assessment submissions
...
BEGIN
  ...
  -- ✅ FIXED: Remove ::uuid cast since assessment_responses.item_id is TEXT
  ...
  SELECT p_session_id, response ->> 'itemId', response ->> 'module',
    ...
    (response ->> 'focusBlurCount')::INTEGER, response ->> 'chosenOption'
  ...
  SELECT jsonb_object_agg(module, jsonb_build_object('total', COUNT(*)::INTEGER,
    'correct', COUNT(CASE WHEN is_correct = true THEN 1 END)::INTEGER))
  INTO v_module_breakdown FROM assessment_responses WHERE session_id = p_session_id GROUP BY module;
