...
-- =====================================================
-- CAT Algorithm Flow:
-- 1. Fetch items: SELECT * FROM irt_item_bank WHERE language = $1 AND is_active = true
-- 2. Apply a-Stratification based on discrimination parameter
-- 3. Select next item using Maximum Fisher Information
-- 4. Update theta using Newton-Raphson MLE
-- 5. Check stopping rule (SE <= 0.35 or max questions reached)
--
-- IRT Best Practices Applied:
-- - 3PL model for multiple choice items
-- - a-Stratification prevents overexposure of high-discrimination items
-- - Content balancing across 5 digital literacy categories
-- - Target SE of 0.35 for adequate precision
-- - Theta bounds of [-4, +4] for stable estimation
