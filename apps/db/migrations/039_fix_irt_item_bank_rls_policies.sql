...
COMMENT ON POLICY "irt_item_bank_authenticated_read" ON irt_item_bank IS
  'Allows authenticated users to read active, approved items for assessments';

COMMENT ON POLICY "irt_item_bank_anon_read" ON irt_item_bank IS
  'Allows anonymous/guest users to read active, approved items for guest assessments';

COMMENT ON POLICY "irt_item_bank_service_all" ON irt_item_bank IS
  'Allows service_role full access for server-side admin operations';

COMMENT ON POLICY "irt_item_bank_admin_all" ON irt_item_bank IS
  'Allows admin users full access to manage item bank';
